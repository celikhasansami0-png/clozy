-- Voltly — Solar EPC project management schema
...
create table public.crew_members (
  ...
  role        text not null default 'Solar Installer',
...
begin
  ...
  -- Projects (solar EPC sites)
  insert into public.jobs (owner_id,name,color,status,phase,completion) values
    (p_owner_id,'Cedar Ridge Solar Farm — 12 MW','#F5A623','In Progress','Construction',58) returning id into j1;
  insert into public.jobs (owner_id,name,color,status,phase,completion) values
    (p_owner_id,'Harbor Logistics Rooftop — 2.4 MW','#60a5fa','On Track','Commissioning',82) returning id into j2;
  insert into public.jobs (owner_id,name,color,status,phase,completion) values
    (p_owner_id,'Mesa Verde Community Solar — 5 MW','#f87171','Delayed','Engineering',21) returning id into j3;
  insert into public.jobs (owner_id,name,color,status,phase,completion) values
    (p_owner_id,'Westfield Distribution Center — 1.8 MW','#4ade80','On Track','Procurement',35) returning id into j4;
  ...
  insert into public.tasks (job_id,owner_id,title,status,priority,assignee_id,due_date,tag) values
    ...
    (j1,p_owner_id,'Submit interconnection revision to utility','todo','urgent',null,'2026-06-13','Interconnection'),
  ...
  insert into public.tasks (job_id,owner_id,title,status,priority,assignee_id,due_date,tag) values
    ...
    (j3,p_owner_id,'Coordinate utility interconnection study','todo','urgent',null,'2026-06-17','Interconnection'),
  ...
  -- Permits (solar EPC permit types)
  insert into public.permits (job_id,owner_id,permit_number,type,status,submitted_date,notes) values
    (j1,p_owner_id,'P-26-8821','Interconnection','Under Review','2026-06-03','Revision requested — string layout detail'),

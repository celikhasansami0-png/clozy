...
const ORBIT_SYSTEM = `You are @Orbit, the in-app AI assistant for Orbit — a project
...
/** @Orbit chat — real Anthropic response, or a graceful mock when no key is set. */
export async function orbitChat(messages: ChatMessage[], context?: string): Promise<string> {
  if (!aiEnabled()) return mockChat(messages, context)
  const system = context ? `${ORBIT_SYSTEM}\n\n## Current workspace\n${context}` : ORBIT_SYSTEM
...
function mockChat(messages: ChatMessage[], context?: string): string {
  ...
  return [
    `(@Orbit demo mode — set ANTHROPIC_API_KEY for live answers.)`,
    ``,
